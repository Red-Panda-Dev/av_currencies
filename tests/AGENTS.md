...
```
tests/
├── parse.test.js      # Vitest tests for lib/rates.js (pure functions, no mocks needed)
└── content.test.js    # Vitest + jsdom tests for content/avby.js (browser mocks + DOM fixtures)
```

## Local boundaries and invariants

- **`parse.test.js`** imports directly from `lib/rates.js`. No mocks, no DOM — runs in plain Node.js.
- **`content.test.js`** is more complex:
  ...
  - Reads HTML fixtures from `examples/` (`index.html`, `auto_card.html`). These are real AV.by page snapshots.
...
- When adding tests for new `lib/rates.js` functions, add them to `parse.test.js`.
...
- `examples/index.html`, `examples/auto_card.html` — AV.by page fixtures (used by `content.test.js`)
